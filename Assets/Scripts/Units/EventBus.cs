...
public class EventBus : IService
{
    ...
    public void Subscribe<T>(Action<T> callback, int priority = 0)
    {
        ...
        _signalCallbacks[key] = _signalCallbacks[key].OrderByDescending(item => item.priority).ToList();
    }
    public void Unsubscribe<T>(Action<T> callback) 
    {
        ...
        if (_signalCallbacks.ContainsKey(key))
        {
            var callbackToDelete = _signalCallbacks[key].FirstOrDefault(item => item.callback.Equals(callback));
